...
from shivalika.api.analyzer import SkillAnalyzer
from shivalika.api.registry import skill_objects
from shivalika.core.nlp import ResponseCreator
from shivalika.api.collection.activation import ActivationSkills
from shivalika.utils.mongoDB import db
from shivalika.api.collection.wolframalpha import WolframSkills
...
class Processor:
    ...
    def run(self):
        ...
        transcript = shivalika.input_engine.recognize_input()
        ...
        if skill:
            ...
            shivalika.output_engine.assistant_response(response)
    ...
    def _execute_skill(self, skill):
        if skill:
            ...
            self.console_manager.console_output(
                info_log="Executing skill {0}".format(skill_func_name)
            )
            try:
                ...
                self.console_manager.console_output(
                    error_log="Failed to execute skill {0} with message: {1}".format(
                        skill_func_name, e
                    )
                )
