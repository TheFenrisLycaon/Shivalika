...
class TTSEngine(TTS):
    ...
    def assistant_response(self, message, refresh_console=True):
        ...
        try:
            speech_tread = threading.Thread(
                target=self._speech_and_console, args=(refresh_console,)
            )
            speech_tread.start()
        except RuntimeError as e:
            self.logger.error(
                "Error in assistant response thread with message {0}".format(e)
            )
    ...
    def _insert_into_message_queue(self, message):
        try:
            ...
            self.logger.error(
                "Unable to insert message to queue with error message: {0}".format(e)
            )
                ...
                if message:
                    ...
                    for batch in batches:
                        ...
                        self.console_manager.console_output(
                            cumulative_batch, refresh_console=refresh_console
                        )
                        self.run_engine()
