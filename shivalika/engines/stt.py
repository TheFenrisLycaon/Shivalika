...
class STTEngine:
    ...
    def __init__(self):
        ...
        self.console_manager.console_output(
            info_log="Microphone configured successfully!"
        )
    def recognize_input(self, already_activated=False):
        ...
        while True:
            ...
            if already_activated or self._activation_name_exist(transcript):
                ...
                return transcript
    def _recognize_speech_from_mic(
        self,
    ):
        """
        ...
        try:
            ...
            self.console_manager.console_output(
                info_log="User said: {0}".format(transcript)
            )
        except sr.UnknownValueError:
            # speech was unintelligible
            transcript = ""
            self.console_manager.console_output(
                info_log="Unable to recognize speech", refresh_console=False
            )
    ...
    def _activation_name_exist(transcript):
        ...
        if transcript:
            ...
            return bool(set(transcript_words).intersection([shivalika.assistant_name]))
        else:
            return False
    @staticmethod
    def _remove_activation_word(transcript):
        transcript = transcript.replace(shivalika.assistant_name, "")
        return transcript
