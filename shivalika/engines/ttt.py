...
from shivalika.utils.console import user_input
...
class TTTEngine:
    ...
    def recognize_input(self, **kwargs):
        ...
        try:
            ...
            self.console_manager.console_output(
                error_log="Failed to recognize user input with message: {0}".format(e)
            )
    ...
    def assistant_response(self, message, refresh_console=True):
        ...
        try:
            if message:
                self.console_manager.console_output(
                    message, refresh_console=refresh_console
                )
        except RuntimeError as e:
            self.console_manager.console_output(
                error_log="Error in assistant response with message: {0}".format(e)
            )
