import logging
from shivalika.utils.mapping import math_symbols_mapping
...
class SkillAnalyzer:
    ...
    def skills(self):
        return (
            db.get_documents(collection="control_skills")
    ...
    def extract(self, user_transcript):
        ...
        user_transcript_with_replaced_math_symbols = (
            self._replace_math_symbols_with_words(user_transcript)
        )

        test_tdm = self.vectorizer.transform(
            [user_transcript_with_replaced_math_symbols]
        )
        ...
        skill_index = similarities.argsort(axis=None)[
            -1
        ]  # Extract the most similar skill
        if similarities[skill_index] > self.analyzer_sensitivity:
            skill_key = [
                skill for skill in enumerate(self.skills) if skill[0] == skill_index
            ][0][1]
            return skill_key
        else:
