    ...
    def _create_response(cls, documents):
        ...
        try:
            for document in documents:
                response += response_base.format(
                    document.get("user_transcript", "--"),
                    document.get("response", "--"),
                    document.get("executed_skill").get("skill").get("name")
                    if document.get("executed_skill")
                    else "--",
                )
        except Exception as e:
            cls.console(error_log=e)
        finally:
            from shivalika.utils import input, console
