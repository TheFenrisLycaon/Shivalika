...
from shivalika.settings import WOLFRAMALPHA_API
        ...
        if voice_transcript:
            try:
                if WOLFRAMALPHA_API["key"]:
                    cls.console(
                        info_log="Wolfarm APi call with query message: {0}".format(
                            voice_transcript
                        )
                    )
                ...
                if InternetSkills.internet_availability():
                    ...
                    cls.console(
                        error_log="There is no result from Wolfram API with error: {0}".format(
                            e
                        )
                    )
                else:
