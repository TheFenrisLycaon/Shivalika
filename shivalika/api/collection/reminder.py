...
from shivalika.utils.input import validate_digits_input
from shivalika.api.skill import AssistantSkill
from shivalika.utils.console import OutputStyler

time_intervals = {
    "seconds": {
        "variations": ["sec", "second", "seconds"],
        "scheduler_interval": "seconds",
    },
    "minutes": {"variations": ["minute", "minutes"], "scheduler_interval": "minutes"},
    "hours": {"variations": ["hour", "hours"], "scheduler_interval": "hours"},
    "months": {"variations": ["month", "months"], "scheduler_interval": "months"},
    "years": {"variations": ["year", "years"], "scheduler_interval": "years"},
}
...
class ReminderSkills(AssistantSkill):
    ...
    def create_reminder(cls, voice_transcript, **kwargs):
        ...
        (
            reminder_duration,
            scheduler_interval,
        ) = cls._get_reminder_duration_and_time_interval(voice_transcript)

        def reminder():
            cls.response(
                "Hey, I remind you that now the {0} {1} passed!".format(
                    reminder_duration, scheduler_interval
                )
            )
            job.remove()
        ...
        try:
            if reminder_duration:
                ...
                cls.response(
                    "I have created a reminder in {0} {1}".format(
                        reminder_duration, scheduler_interval
                    )
                )
                scheduler.start()
    ...
    def set_alarm(cls, voice_transcript, **kwargs):
        ...
        alarm_hour = validate_digits_input(
            message="Tell me the exact hour", values_range=[0, 24]
        )
        alarm_minutes = validate_digits_input(
            message="Now tell me the minutes", values_range=[0, 59]
        )

        try:
            thread = Thread(
                target=cls._alarm_countdown, args=(alarm_hour, alarm_minutes)
            )
            thread.start()
        except Exception as e:
            cls.console(
                error_log="Failed to play alarm with error message: {0}".format(e)
            )
    ...
    def _alarm_countdown(cls, alarm_hour, alarm_minutes):
        ...
        alarm_time = datetime.datetime.combine(
            now.date(), datetime.time(alarm_hour, alarm_minutes, 0)
        )
        ...
        if waiting_period < datetime.timedelta(0):
            ...
            response_message = (
                "Alarm - {0}:{1} for today is configured "
                + OutputStyler.GREEN
                + "successfully!"
                + OutputStyler.ENDC
            )
            ...
            cls.response(
                "Wake up! It's {0}".format(datetime.datetime.now().strftime("%H:%M"))
            )

            skills_dir = os.path.dirname(__file__)
            alarm_soundfile = os.path.join(
                skills_dir, "..", "files", "analog_watch_alarm.wav"
            )
