...
from shivalika.enumerations import InputMode, MongoCollections
from shivalika.utils import console
from shivalika.utils import input

input_mode = db.get_documents(collection="general_settings")[0]["input_mode"]
response_in_speech = db.get_documents(collection="general_settings")[0][
    "response_in_speech"
]
...
class ConfigurationSkills(AssistantSkill):
    ...
    def configure_assistant(cls, **kwargs):
        ...
        cls.console(
            "NOTE: Current name: {0}".format(assistant_name), refresh_console=False
        )
        console.headerize()
        ...
        new_input_mode = input.validate_input_with_choices(
