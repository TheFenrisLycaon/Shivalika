...
class LinuxAppSkills(AssistantSkill):
    ...
    def open_new_bash(cls, **kwargs):
        ...
        try:
            subprocess.Popen(
                ["gnome-terminal"], stderr=subprocess.PIPE, shell=False
            ).communicate()
    ...
    def open_note_app(cls, **kwargs):
        ...
        try:
            subprocess.Popen(
                ["gedit"], stderr=subprocess.PIPE, shell=False
            ).communicate()
        except FileNotFoundError:
            cls.response("You don't have installed the gedit")
            time.sleep(2)
            cls.response(
                "Install gedit with the following command: 'sudo apt-get install gedit'"
            )
    ...
    def open_new_browser_window(cls, **kwargs):
        ...
        try:
            subprocess.Popen(
                ["firefox"], stderr=subprocess.PIPE, shell=False
            ).communicate()
        except Exception as e:
