...
from shivalika.utils.console import headerize
...
class AssistantInfoSkills(AssistantSkill):
    ...
    def tell_the_skills(cls, **kwargs):
        ...
        try:
            ...
            cls.console(
                error_log="Error with the execution of skill with message {0}".format(e)
            )
    ...
    def _create_skill_response(cls, response):
        ...
        for skill_id, skill in enumerate(basic_skills, start=1):
            response = response + basic_skills_body_format.format(
                skill_id, skill.get("name"), skill.get("description"), skill.get("tags")
            )
        ...
        for skill_id, skill in enumerate(skills, start=1):
            response = response + learned_skills_body_format.format(
                skill_id, skill.get("name"), skill.get("tags"), skill.get("response")
            )
