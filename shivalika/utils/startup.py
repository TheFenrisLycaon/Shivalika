...
from shivalika.utils import console
from shivalika.enumerations import MongoCollections
from shivalika.core.console import ConsoleManager
...
def play_activation_sound():
    ...
    activation_soundfile = os.path.join(
        utils_dir, "..", "files", "activation_sound.wav"
    )
...
def internet_connectivity_check(url="http://www.google.com/", timeout=2):
    ...
    try:
        ...
        console_manager.console_output(
            warn_log="Skills with internet connection will not work"
        )
        return False
...
def configure_MongoDB(db, settings):
    ...
    if db.is_collection_empty(collection=MongoCollections.GENERAL_SETTINGS.value):
        ...
        try:
            db.update_collection(
                ...
                documents=[new_settings],
            )
            console.print_console_header("Assistant Name")
            print(
                "Assistant name- {0} configured successfully!".format(
                    default_assistant_name.lower()
                )
            )
            print(
                "Input mode - {0} configured successfully!".format(default_input_mode)
            )
            print(
                "Speech response output- {0} configured successfully!".format(
                    default_response_in_speech
                )
            )
            time.sleep(2)
            ...
            logging.error(
                "Failed to configure assistant settings with error message {0}".format(
                    e
                )
            )
    ...
    from shivalika.api.registry import CONTROL_SKILLS, ENABLED_BASIC_SKILLS
