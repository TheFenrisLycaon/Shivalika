...
def validate_digits_input(message, values_range=None):
    ...
    input_number = None
    while True:
        shivalika.output_engine.assistant_response(message)
        user_input = shivalika.input_engine.recognize_input(already_activated=True)
        try:
        ...
        if values_range:
            ...
            if not min_value <= input_number <= max_value:
                shivalika.output_engine.assistant_response(
                    "Please give a number higher/equal than {0} and smaller/equal than {1}".format(
                        min_value, max_value
                    )
                )
                raise ValueError
...
def check_input_to_continue(message=""):
    ...
    return (
        shivalika.input_engine.recognize_input(already_activated=True)
        in positive_answers
    )
...
def validate_input_with_choices(available_choices):
    ...
    while not user_input in available_choices:
        shivalika.output_engine.assistant_response(
            "Please select on of the values: {0}".format(available_choices)
        )
        user_input = shivalika.input_engine.recognize_input(already_activated=True)
    return user_input
